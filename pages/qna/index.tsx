...
function HomeComposerStrip() {
  ...
  // ✅ NEW: disable button while saving
  ...
  // ✅ NEW: homepage Ask -> write to qna_questions and redirect to /qna
  const submitAskToQnA = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("Homepage ask -> qna_questions insert error:", error);
        return;
      }
      ...
      if (data?.id) router.push(`/qna?open=${data.id}`);
          ...
          <div style={modalCard}>
            ...
            <div style={footerBar}>
              ...
              <button
                ...
                disabled={!canSubmit}
                onClick={() => {
                  // ✅ ONLY CHANGE: Ask goes to qna_questions
                  if (mode === "post") {
                    // ignore posts for now — as requested
                    setPostText("");
                    closeComposer();
                    return;
                  }
              ...
              >
                {mode === "post" ? "Post" : askSaving ? "Posting…" : "Ask"}
              </button>
            </div>
          </div>
        </div>
