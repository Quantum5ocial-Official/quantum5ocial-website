...
function QnAMiddle() {
  ...
  // ✅ NEW: keep track if we already auto-opened from URL once
  const [didAutoOpenFromUrl, setDidAutoOpenFromUrl] = useState(false);
  ...
  // ✅ NEW: Auto-open thread from ?open=<id> (homepage redirects here)
  useEffect(() => {
    if (!router.isReady) return;
    if (didAutoOpenFromUrl) return;
    ...
    if (!openId) return;
    if (openQ) {
      setDidAutoOpenFromUrl(true);
      return;
    }

    const tryOpen = async () => {
      // 1) try from already-loaded list first
      const fromList = questions.find((q) => q.id === openId);
      if (fromList) {
        await openThread(fromList);
        setDidAutoOpenFromUrl(true);

        // clean URL so refresh doesn't re-open
        const nextQuery = { ...router.query };
        delete (nextQuery as any).open;
        router.replace({ pathname: "/qna", query: nextQuery }, undefined, { shallow: true });

        return;
      }

      // 2) otherwise fetch that single question (even if list is filtered)
      const { data, error } = await supabase
        .from("qna_questions")
        .select(
          `
      ...
      if (error) {
        console.error("auto-open fetch question error:", error);
        setDidAutoOpenFromUrl(true);
        return;
      }
      ...
      if (data) {
        ...
        // also prepend into list if it's not already there (keeps UI consistent)
        setQuestions((prev) => {
          if (prev.some((q) => q.id === normalized.id)) return prev;
    ...
    // If questions haven't loaded yet, wait until they do (but still fetch fallback above if needed)
    // We'll just run immediately; it can open from list or fetch.
    tryOpen();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [router.isReady, router.query.open, didAutoOpenFromUrl, questions]);
